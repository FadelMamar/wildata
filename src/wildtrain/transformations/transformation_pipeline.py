...
class TransformationPipeline:
    ...
    def transform(self, inputs: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for i, transformer in enumerate(self.transformers):
            try:
                self.logger.debug(f"Applying transformer {i+1}/{len(self.transformers)}: {transformer.__class__.__name__}")
    ...
    def transform_batch(self, inputs: List[Dict[str, Any]]) -> List[List[Dict[str, Any]]]:
        ...
        for i, data in enumerate(inputs):
            try:
                ...
                self.logger.error(f"Error transforming image {i+1}: {traceback.format_exc()}")
                raise
