...
class AugmentationTransformer(BaseTransformer):
    ...
    def _transform_once(self, inputs:Dict[str, Any])->List[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.warning(f"Augmentation failed, returning original data: {traceback.format_exc()}")
            #augmented_image = image
            #transformed_annotations = annotations
            raise e
  
    def _prepare_albumentations_data(self, image: np.ndarray, annotations: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def _process_augmented_annotations(self, augmented_data: Dict[str, Any], 
                                     original_annotations: List[Dict[str, Any]]) -> Tuple[np.ndarray, List[Dict[str, Any]]]:
