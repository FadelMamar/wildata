...
class YOLOAdapter(BaseAdapter):
    ...
    def convert(self, split: str) -> Dict[str, List[str]]:
        ...
        annotations = self._filter_annotations_by_image_ids(set(image_id_to_image.keys()))
    ...
    def _annotation_to_yolo_line(self, ann: Dict[str, Any], width: int, height: int) -> str:
        ...
        if 'segmentation' in ann and ann['segmentation']:
            ...
            if isinstance(seg, list) and seg and isinstance(seg[0], (list, float, int)):
                ...
                if isinstance(seg[0], list):
                    seg_points = [str(float(pt) / width if i % 2 == 0 else float(pt) / height)
                                  for poly in seg for i, pt in enumerate(poly)]
                else:
                    seg_points = [str(float(pt) / width if i % 2 == 0 else float(pt) / height)
