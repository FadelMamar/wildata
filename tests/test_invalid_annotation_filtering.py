...
def create_invalid_master_data():
    ...
    return {
        "dataset_info": {
            ...
            "schema_version": "1.0",
            "task_type": "detection",
            "classes": [{"id": 1, "name": "test_category", "supercategory": "test"}],
        },
        "images": [
            {
                "id": 1,
...
class TestInvalidAnnotationFiltering:
    ...
    def test_master_validator_with_filtering(self):
        ...
        assert any(
            "references non-existent image_id" in s.get("reason", "") for s in skipped
        )
        assert any(
            "references non-existent category_id" in s.get("reason", "")
            for s in skipped
        )
    ...
    def test_coco_converter_with_filtering(self):
        ...
        try:
            ...
            master_data = converter.convert_to_master(
                "test_dataset", filter_invalid_annotations=True
            )
    ...
    def test_yolo_validator_and_converter_with_filtering(self):
        ...
        real_image_path = Path(
            r"D:/workspace/savmap/yolo/train/images/00a033fefe644429a1e0fcffe88f8b39_0_4_0_1024_640_1664.jpg"
        )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert set(img_files[img_id] for img_id in img_ids) == {
                "img1.jpg",
                "img2.jpg",
            }
            # Check bbox for img1.jpg (from img1.txt)
            img1 = next(
                img for img in master_data["images"] if img["file_name"] == "img1.jpg"
            )
            ann1 = next(ann for ann in anns if ann["image_id"] == img1["id"])
            width1, height1 = img1["width"], img1["height"]
            expected_bbox1 = [
                0.5 * width1 - 0.2 * width1 / 2,
                0.5 * height1 - 0.2 * height1 / 2,
                0.2 * width1,
                0.2 * height1,
            ]
            ...
            img2 = next(
                img for img in master_data["images"] if img["file_name"] == "img2.jpg"
            )
            ann2 = next(ann for ann in anns if ann["image_id"] == img2["id"])
            width2, height2 = img2["width"], img2["height"]
            expected_bbox2 = [
                0.5 * width2 - 0.2 * width2 / 2,
                0.5 * height2 - 0.2 * height2 / 2,
                0.2 * width2,
                0.2 * height2,
            ]
