...
class TestDataPipelineSynthetic:
    ...
    def _create_synthetic_coco_data(self):
        coco_data = {
            ...
            "annotations": [
                ...
                {"id": 2, "image_id": 2, "category_id": 1, "bbox": [150, 150, 250, 200], "area": 25000, "iscrowd": 0}
            ],
            "categories": [
    ...
    def test_import_coco_synthetic_data(self):
        ...
        result = pipeline.import_dataset(
            source_path=str(self.coco_test_file), source_format="coco", dataset_name="test_coco_dataset"
        )
...
class TestDataPipelineRealData:
    ...
    def test_import_coco_real_data(self):
        ...
        pipeline = DataPipeline(str(self.test_data_dir))
        result = pipeline.import_dataset(
            source_path=str(self.coco_test_file), source_format="coco", dataset_name="real_coco_dataset"
        )
        assert result['success'] is True
    ...
    def test_import_yolo_real_data(self):
        ...
        assert self.yolo_test_dir.exists(), f"YOLO directory not found: {self.yolo_test_dir}"
        data_yaml = self.yolo_test_dir / "data.yaml"
        assert data_yaml.exists(), f"YOLO data.yaml not found in {self.yolo_test_dir}"
        pipeline = DataPipeline(str(self.test_data_dir))
        result = pipeline.import_dataset(
            source_path=str(data_yaml), source_format="yolo", dataset_name="real_yolo_dataset"
        )
        if result['success']:
            ...
            assert any('must be a string' in err for err in result.get('validation_errors', []))
...
class TestDataPipeline:
    ...
    def test_import_coco_dataset_success(self, mock_converter, mock_validator):
        ...
        mock_converter_instance.convert_to_master.return_value = {
            ...
            "images": [
                {"id": 1, "file_name": "test_image.jpg", "file_path": str(self.images_dir / "test_image.jpg"), "width": 640, "height": 480, "split": "train"}
            ],
            "annotations": [
                {"id": 1, "image_id": 1, "category_id": 1, "bbox": [100, 100, 200, 150], "type": "detection"}
            ]
        }
        mock_converter.return_value = mock_converter_instance
        pipeline = DataPipeline(str(self.project_root))
        result = pipeline.import_dataset(
            source_path=str(self.coco_dir / "annotations.json"), source_format="coco", dataset_name="test_dataset"
        )
        assert result['success'] is True
    @patch('wildtrain.pipeline.data_pipeline.COCOValidator')
...
class TestMasterDataManager:
    ...
    def test_store_dataset(self):
        master_data = {
            ...
            "images": [
                {"id": 1, "file_name": "test_image.jpg", "file_path": str(self.project_root / "test_image.jpg"), "width": 640, "height": 480, "split": "train"}
            ],
            "annotations": [
                {"id": 1, "image_id": 1, "category_id": 1, "bbox": [100, 100, 200, 150], "type": "detection"}
            ]
...
class TestFrameworkDataManager:
    ...
    def setup_method(self):
        ...
        master_data = {
            ...
            "images": [
                {"id": 1, "file_name": "test_image.jpg", "file_path": str(self.master_images_dir / "train" / "test_image.jpg"), "width": 640, "height": 480, "split": "train"}
            ],
            "annotations": [
                {"id": 1, "image_id": 1, "category_id": 1, "bbox": [100, 100, 200, 150], "type": "detection"}
            ]
